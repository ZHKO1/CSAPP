...
void doit(int fd, pthread_t tid);
...
void sigchild_handler(int sig)
{
    ...
    {
        if (WIFEXITED(status))
            printf("pid %d terminated normally with exit status=%d\n", pid, WEXITSTATUS(status));
        else
...
void doit(int fd, pthread_t tid)
{
    ...
    if (rc == 0)
    {
        printf("tid(%ld): fd(%d) not send any bytes\n", tid, fd);
        fflush(stdout);
        return;
    }
